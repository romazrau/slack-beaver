...
- `agent-retrieval-reviewer.md`: Implemented retrieval planning and independent reviewer agent behavior for `ask` and App DM quality control, clarification follow-up handling for short preference replies, bounded/deduplicated planner query variants, bounded typed supplemental reads when reviewer context is insufficient, and an internal expanded single-document Google Drive retrieval budget for clear whole-document outline requests.
- `agent-retrieval-fallback-optimization.md`: Two-goal optimization for reducing generic insufficient-context fallbacks, with configured-context retrieval reliability implemented and public web/Google web search explicitly kept out of scope.
- `agent-retrieval-candidate-reuse.md`: Implemented structured recent retrieval candidate memory, stale clarification hygiene, candidate-seeded Drive retries, and multi-candidate user choice handling.
- `agent-workflow-state-machine.md`: Decision to use a shared conversation-level workflow state machine for retrieval, future file writing/editing, research organization, and remote task flows instead of a search-only state machine.
