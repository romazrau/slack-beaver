...
- Public web/Google search is intentionally not enabled; public-web requests are answered with the boundary and an option to search configured local files, Google Drive, and Gmail instead.
...
- Retrieval answers from `ask <question>` and natural App DM are reviewed before Slack delivery when tool context was used; subjective short-passage requests and planner clarification follow-ups carry short user replies back into the original request, planner source/query variants are bounded and deduplicated before execution, content-like files are preferred over README/planning search-hint files for typed reads without demoting ordinary `docs/` content, reviewer follow-up instructions are not returned directly to Slack, and over-budget tool loops can pause with partial results, explicitly ask whether to continue, route replies through a no-tool confirmation classifier, then resume only on clear `continue` / `繼續`, stop on cancellation, or clear the pending run when a new request is handled.
- Clear single-document requests such as complete outlines or full-article summaries can use an internal expanded retrieval budget for Google Drive Docs/PDF reads. The Drive read tool accepts an optional extracted-text `offset` and returns `nextOffset` when content is still truncated, so the agent can continue reading later segments of the same Google Doc or PDF without restarting from the beginning. The app still enforces bounded reads with an 80,000-character hard cap per read.
...
- Agent loop trace logs are written under ignored `logs/agent-traces/` JSONL files with tool-call inputs, bounded result summaries, tool-call error summaries, fallback reasons, and reviewer decisions for local debugging.
