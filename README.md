...
- Clear single-document requests such as complete outlines or full-article summaries can use an internal expanded retrieval budget for Google Drive Docs/PDF reads. The public tool schema still accepts only document IDs; the app decides whether to raise the Drive read bound up to an 80,000-character hard cap and whether to allow a small number of extra tool turns. Very large documents may still be truncated and should be handled with segmented summaries or a future document index when full coverage is required.
