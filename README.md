...
下一階段已加入 Local Memory + OpenAI agent runner 的第一個可驗收切片：Local Agent 會用 SQLite 記住使用者允許讀取的 local folders，沒有特別指定路徑時使用已知 allowed folders，並透過受限 Tool Registry 執行 local search。OpenAI token 可透過本機 CLI 儲存；Slack 內貼上的 token-like 訊息會被拒絕。`ask <question>` 會呼叫 OpenAI Responses API，但 model 只能使用已註冊且經驗證的 `local_search` tool。
...
2. Messages tab 輸入 `find Socket`、`find onboarding`，或 `ask What does the deployment checklist say?`。
3. Local Agent 透過 Socket Mode 收到 `message.im` event。
4. `find` 會直接搜尋 allowlisted local folders；`ask` 會透過 OpenAI agent runner 只能調用 guarded Tool Registry。
...
每次 App Home message search/ask 會在 audit log 記錄 `source=app_home_message`。Slash command search/ask 會記錄 `source=slash_command`。
...
4. 回到 Slack 輸入 `find <query>` 或 `ask <question>`

完成 folders 和 OpenAI token 設定後，`ask <question>` 會呼叫 OpenAI Responses API，且 model 只能透過 guarded Tool Registry 使用 `local_search`。`find <query>` 仍是 deterministic local search，不會呼叫 OpenAI。
...
OpenAI provider 會透過 local token file 呼叫 OpenAI Responses API，不會把 token 送進 Slack 或 audit log。AI token 是付費機密，不能透過 Slack DM、App Home message、文件或 audit log 傳遞。請只在使用者電腦上透過 local CLI 設定：

```sh
npm run agent:secrets:set-openai
```

預設 token path 是 `./tokens/openai.key`，由 `.gitignore` 保護，寫入權限為 `0600`。Slack 中若收到 token-like 字串，bot 會拒絕接收並提示使用 local CLI。`OPENAI_MODEL` 預設為 `gpt-4.1-mini`，`MAX_AGENT_TOOL_TURNS` 預設為 `2`。
...
- OpenAI agent runner 只能執行已註冊的 `local_search` tool，且 tool input schema 不接受 path、shell command 或 write request。
...
尚未實作：多 provider routing、AI summary memory、embeddings/index cache、Google Workspace tool calls。
